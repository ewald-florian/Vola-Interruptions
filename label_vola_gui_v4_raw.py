...
class SampleLabelerGUI(tk.Tk):
    def __init__(self,
        ...
        self.data_dir = os.path.join(script_dir, "vola_data_midpoints_raw")
        self.target_dir = os.path.join(script_dir,
                                       "labeled_data/labels_raw_midpoints")
    ...
    def _plot_vola(self, df):
        ...
        coeff_before = np.polyfit(pre_vola.index, pre_vola["Midpoint"], 1)
        coeff_after = np.polyfit(post_vola.index, post_vola["Midpoint"], 1)

        line_before = np.polyval(coeff_before, pre_vola.index)
        line_after = np.polyval(coeff_after, post_vola.index)

        plt.plot(df["Midpoint"], label="Midpoint", color="b")
